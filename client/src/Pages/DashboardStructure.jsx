...
import CourseStructure from "../components/dashboard/CourseStructure";
...
const DashboardStructure = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (result.success && result.data && result.data.structure) {
  ...
  const handleNext = async () => {
    setLoading(true);
    try {
      const result = await saveDashboardData('structure', { structure });
      ...
      <main style={{ flex: 1, padding: '48px 48px 48px 48px', maxWidth: 900, margin: '0 auto', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', position: 'fixed', bottom: 40, right: 48 }}>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? 'Saving...' : 'Next: Setup & Test Video'}
          </button>
        </div>
      </main>
    </div>
